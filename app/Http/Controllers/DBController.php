...
///// // }
    ...
    function add_question(Request $req) {
        ...
        else if($req->Choose =="youtube_url"){
        ...
        $newQuestion->Model_setting_position_x =35;
        $newQuestion->Model_setting_position_y =-119;
        $newQuestion->Model_setting_position_z =-55;
        $newQuestion->Model_setting_size =100;
        //+
        $newQuestion->youtube_url =$youtube_url;
        //+
        ///////新增/////////
    ...
    function add_question_csv(Request $req) {
        ...
        $delimiter = ',';
        $headerSample = ['question_level','image_url','question','hint_1','hint_2','answer','chinese_answer','Choose','Youtube_url', 'Question_model'];
        $header = null;
        $data = array();
        ...
        foreach($data as $d)
        {
            ...
            if($Choose =="model"){
                ...
                if (!file_exists($file_model_location)) {
                    ...
                    $new_model_url =null;
                    //缺一個警告
                    throw ValidationException::withMessages(['uploadCSV' => "model為空"]);
                }
                else{
                    //讀取整個文件進入一個字串符中
            ...
            else if($Choose =="Youtube"){
                $Choose = "youtube_url";
                $youtube_url = $d['Youtube_url'];
                $new_model_url = null;
                if($youtube_url == null){
                    throw ValidationException::withMessages(['uploadCSV' => "Youtube為空"]);
                }
            }
            else if($Choose == null){
                $new_model_url = null;
                ...
                $Choose == null;
                throw ValidationException::withMessages(['uploadCSV' => "Choose欄位錯誤"]);
            }
    ...
    function delete_episode(Request $req) {
        ...
        $newepisode_participants = array_map('intval', explode(',', str_replace(array( '[', ']' ), '', $old_episode->participants)));
        
        foreach($newepisode_participants as $ep) {
            if($ep!=0){
                $user = DB::table('users')
                ->where('user_id', '=', $ep)
                ->first();
                ...
                $newuser_episode_array = array_map('intval', explode(',', str_replace(array( '[', ']' ), '', $user->user_episode)));
                
                array_splice($newuser_episode_array, array_search($episode_id, $newuser_episode_array), 1);
                $newuser_episode_string = "[";

                foreach($newuser_episode_array as $choose) {
                    $newuser_episode_string = $newuser_episode_string.strval($choose).",";
                }
                
                $newuser_episode_string = rtrim($newuser_episode_string, ", ")."]";
                echo $newuser_episode_string."  ";
                $user = DB::table('users')
                    ->where('user_id', '=', $ep)
                    ->update(['user_episode' => $newuser_episode_string]);
        ...
        ///////////刪除stroage下的圖片/////////////
        $old_question = DB::table('questions')->where('questions.episode_id', $episode_id)->get();
        foreach($old_question as $value ){
            //+
            $Original_question_imageName = $value->Original_question_imageName;
            $old_jsonName = $Original_question_imageName.".json";
            Storage::delete($old_jsonName);

            $old_model_url = $value->model_url;
            if($old_model_url != null){
                $old_model_name = substr($old_model_url, strrpos($old_model_url, "/") + 1);
                Storage::delete('public/model/'.$old_model_name);
            }
            //+
            
            $old_question_imageurl =$value->image_url;
            //
            $vuforia_id=$value->vuforia_id;
            $vuforiaClient = new VuforiaController;
        ...
        //Update user's user_episode
        //first find participants
    ...
    function delete_question(Request $req) {
        ...
        ///////////刪除stroage下的圖片/////////////
        $old_question_de = DB::table('questions')->where('episode_id', '=', $req->episode_id)
        ->where('lesson_id', $req->lesson_id)->where('question_id', $req->question_id);
        $old_question = $old_question_de->first();
        ...
        ///////////////////////////////////////////////////////
        //+
        $Original_question_imageName = $old_question->Original_question_imageName;
        $old_jsonName = $Original_question_imageName.".json";
        Storage::delete($old_jsonName);

        $old_model_url = $old_question->model_url;
        if($old_model_url != null){
            $old_model_name = substr($old_model_url, strrpos($old_model_url, "/") + 1);
            Storage::delete('public/model/'.$old_model_name);
        }
        ...
        $vuforia_id = $old_question->vuforia_id;
        $vuforiaClient = new VuforiaController;
        $vuforiaResponse = $vuforiaClient->deleteTarget($vuforia_id);
        $old_question_de->delete();
        return redirect()->route('question', [
            'episode_id' => $req->episode_id,
            'lesson_id' => $req->lesson_id,
            'lesson_name' => $req->lesson_name
    ...
    function edit_question(Request $req){
        ...
        if($req->Choose =="model"){
            ...
            if($req->hasfile('model_url')){
                //刪除舊model(如果有)
                $old_model_url = $question_info->model_url;
                if($old_model_url != null){
                    $old_model_name = substr($old_model_url, strrpos($old_model_url, "/") + 1);
                    Storage::delete('public/model/'.$old_model_name);
                }
